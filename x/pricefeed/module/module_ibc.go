...
func (im IBCModule) OnRecvPacket(
	...
	var modulePacketData types.PricefeedPacketData
	...
	case *types.PricefeedPacketData_OracleRequestPacket:
		packetAck, err := im.keeper.OnRecvOracleRequestPacket(ctx, modulePacket, *packet.OracleRequestPacket)
		if err != nil {
		...
		sdkCtx.EventManager().EmitEvent(
			sdk.NewEvent(
				types.EventTypeOracleRequestPacket,
				sdk.NewAttribute(sdk.AttributeKeyModule, types.ModuleName),
				sdk.NewAttribute(types.AttributeKeyAckSuccess, fmt.Sprintf("%t", err != nil)),
			),
		)

	case *types.PricefeedPacketData_OracleResponsePacket:
		packetAck, err := im.keeper.OnRecvOracleResponsePacket(ctx, modulePacket, *packet.OracleResponsePacket)
		if err != nil {
			ack = channeltypes.NewErrorAcknowledgement(err)
		} else {
			// Encode packet acknowledgment
...
func (im IBCModule) OnAcknowledgementPacket(
	...
	switch packet := modulePacketData.Packet.(type) {
	case *types.PricefeedPacketData_OracleRequestPacket:
		err := im.keeper.OnAcknowledgementOracleRequestPacket(ctx, modulePacket, *packet.OracleRequestPacket, ack)
		if err != nil {
			return err
		}
		eventType = types.EventTypeOracleRequestPacket
	case *types.PricefeedPacketData_OracleResponsePacket:
		err := im.keeper.OnAcknowledgementOracleResponsePacket(ctx, modulePacket, *packet.OracleResponsePacket, ack)
		if err != nil {
			return err
		}
		eventType = types.EventTypeOracleResponsePacket
...
func (im IBCModule) OnTimeoutPacket(
	...
	switch packet := modulePacketData.Packet.(type) {
	case *types.PricefeedPacketData_OracleRequestPacket:
		err := im.keeper.OnTimeoutOracleRequestPacket(ctx, modulePacket, *packet.OracleRequestPacket)
		if err != nil {
			return err
		}
	case *types.PricefeedPacketData_OracleResponsePacket:
		err := im.keeper.OnTimeoutOracleResponsePacket(ctx, modulePacket, *packet.OracleResponsePacket)
		if err != nil {
			return err
		}
