...
GOV_ADDR=$(bitorad q auth module-account gov -o json 2>/dev/null | jq -r '.account.base_account.address // .base_account.address // empty')
if [[ -z "$GOV_ADDR" || "$GOV_ADDR" == "null" ]]; then
  GOV_ADDR=$(bitorad q auth module-accounts -o json 2>/dev/null | jq -r '.accounts[] | select(.name=="gov" or .base_vesting_account.base_account.address!=null) | .base_account.address // .base_vesting_account.base_account.address' | head -n1)
fi
if [[ -z "$GOV_ADDR" || "$GOV_ADDR" == "null" ]]; then
  GOV_ADDR=$(bitorad debug addr "gov" 2>/dev/null || true)
fi
if [[ -z "$GOV_ADDR" || "$GOV_ADDR" == "null" ]]; then
...
bitorad tx gov submit-proposal "$TMP" \
  --from "$FROM" \
  --chain-id "$CHAIN_ID" \
  --node "$NODE" \
  --fees "$FEES" \
